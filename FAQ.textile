...
<h2>Is this semantic? Will I end up with classes like <code>.formYellow</code>?</h2>

OOCSS can be written in a semantic or non-semantic way, it is up to you to create modules like <code>errorMod</code> rather than <code>bigRedModule</code>. The code and docs are a framework, an example of OOCSS. I can't predict what you are going to put in leftCol. I could call it navigation, but that my not be true for your site. I've chosen to use generic class names, and when doing so I had a few goals in mind (in no particular order).
...
Sometimes these <strong>important goals are in opposition to one another</strong>.  In those cases I've fallen back on pragmatism and made a judgement call. Nothing can replace a clever developer making the right choice in a given situation.