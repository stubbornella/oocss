...
In keeping with the OO metaphor, an object is analogous with a class in Java.

A CSS object consists of four things:
# HTML, which can be one or more nodes of the DOM,
# CSS declarations about the style of those nodes all of which begin
with the class name of the wrapper node
# Components like background images and sprites required for display,
and
# JavaScript behaviors, listeners, or methods associated with the
object.

This can be confusing because each CSS class is not necessarily an
object in it’s own right, but can be a property of a wrapper class.
For example:
...
The object is a module, indicated by the class mod. It contains four
property nodes (which cannot live independently from the module,
including two required regions, inner and body, and two optional
regions, head and foot.
...
# Less repaints and layout calculations on the part of the browser.  
** On a single page, the more CSS rules are reused, the less time the rendering engine spends calculating "computed values"
** with the class "extending"  approach to CSS, one tends to end up with less overriding rules, which means again less of a need for the engine to compute with rules to apply.
...
No--descendant selectors are not discouraged, but putting them too high in the DOM tree is. Avoid putting a wide-net class or id way up on the body or outermost divs of a page, and then writing lots of styles to generate variants of objects. It's no reusable, and it slows down page rendering. Instead, make a more "local" variant by adding a class directly onto the object (and add descendent styles to its children).

So something like this is bad:

<code>.BrowseProducts .carousel</code>

where the first class is on the body tag, and "browse products" is a page or area of the site. This makes the browser do a lot of recalculation of "computed values," which slows things down.

But something like:

<code>.myModule .title</code>

is encouraged, because it's object-based--the ancestor element is presumably close enough to the child to minimize performance impact.
